...
export const useFallingDice = () => {
  useEffect(() => {
    const createFallingDice = () => {
      ...
      dice.textContent = diceChars[Math.floor(Math.random() * diceChars.length)];
      dice.style.left = Math.random() * 100 + 'vw';
      dice.style.top = '-100px'; // Começar bem acima da tela visível
      dice.style.color = colors[Math.floor(Math.random() * colors.length)];
      dice.style.fontSize = (Math.random() * 20 + 15) + 'px';
      dice.style.animationDuration = (Math.random() * 10 + 5) + 's';
